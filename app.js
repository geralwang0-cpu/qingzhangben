...
function myCategorySeed() {
  return [
    ...
    { id: 'daily', name: '日用', type: 'expense', color: '#6ea8ff', icon: 'cute-bag', custom: true, aliases: ['日用', '日用品', '生活用品', '家清', '个人护理', '洗护用品', '卫生用品', '身体乳', '润肤乳', '润肤露', '护肤乳', '护肤', '护手霜', '面霜', '乳液', '爽肤水', '化妆水', '精华', '面膜', '防晒霜', '洗面奶', '洁面', '卸妆', '卸妆水', '沐浴露', '洗发水', '护发素', '香皂', '肥皂', '洗手液', '洗洁精', '洗衣液', '洗衣粉', '洗衣凝珠', '柔顺剂', '衣物除菌液', '清洁剂', '消毒液', '湿巾', '棉柔巾', '纸巾', '抽纸', '卫生纸', '卫生巾', '棉签', '牙膏', '牙刷', '毛巾', '浴巾', '抹布', '拖把', '扫把', '垃圾袋', '保鲜膜', '保鲜袋', '收纳', '衣架', '晾衣架', '电池', '灯泡', '插座', '香薰', '空气清新剂', '花露水', '蚊香'] },
...
function migrateCategoryDefinitionsV4(force) {
  if (!force && state.settings.categoryDefinitionsV4) return { updated: false, moved: 0 };
  const seedDaily = myCategorySeed().find((c) => c.id === 'daily');
  let daily = (state.settings.customCategories || []).find((c) => c.id === 'daily');
  if (!daily && seedDaily && state.settings.customMode) {
    daily = Object.assign({}, seedDaily, { aliases: seedDaily.aliases.slice() });
    state.settings.customCategories.push(daily);
  }
  if (daily && seedDaily) daily.aliases = seedDaily.aliases;

  let moved = 0;
  if (daily) {
    for (const record of state.records) {
      if (record.type !== 'expense' || record.category === daily.id) continue;
      const text = normalizeText([record.item, record.merchant, record.note, (record.tags || []).join(' ')].join(' '));
      if (daily.aliases.some((alias) => text.includes(alias))) {
        record.category = daily.id;
        moved += 1;
      }
    }
  }

  state.settings.categoryDefinitionsV4 = true;
  saveState();
  return { updated: !!(daily && seedDaily), moved };
}
...
function loadState() {
  ...
  try {
    ...
    base.settings.categoryDefinitionsV4 = !!(data.settings && data.settings.categoryDefinitionsV4);
...
async function importData(file) {
  try {
    ...
    const importedRecords = data.records.map(normalizeRecord);
    ...
    for (const category of (data.settings && data.settings.customCategories) || []) {
      if (!state.settings.customCategories.some((c) => c.id === category.id)) {
        state.settings.customCategories.push(category);
      }
    }
    const dailyMigration = migrateCategoryDefinitionsV4(true);
    const petMigration = migratePetCategory(true);
    const latestDate = state.records.map((r) => r.date).filter(Boolean).sort().pop();
    if (latestDate) {
      state.month = latestDate.slice(0, 7);
      state.view = 'home';
    }
    saveState();
    initControls();
    render();
    const importParts = [`已合并添加 ${addedCount} 条记录，共 ${state.records.length} 条`];
    if (petMigration.moved) importParts.push(`${petMigration.moved} 条归入「宠物」`);
    if (dailyMigration.moved) importParts.push(`${dailyMigration.moved} 条归入「日用」`);
    if (latestDate) importParts.push('已跳转到最近记账月份');
    toast(importParts.join('，'));
  } catch (err) {
    toast('导入失败，请检查文件');
  }
}
...
function init() {
  ...
  migratedDaily = migrateCategoryDefinitionsV4();
  ...
  else if (migratedDaily.moved) toast(`已把 ${migratedDaily.moved} 条记录归入「日用」`);
  else if (migratedDaily.updated) toast('已更新「日用」分类范围');
