...
function migrateCategoryDefinitionsV4() {
  if (state.settings.categoryDefinitionsV4) return { updated: false, moved: 0 };
  const daily = (state.settings.customCategories || []).find((c) => c.id === 'daily');
...
async function importData(file) {
  try {
    ...
    const dailyMigration = migrateCategoryDefinitionsV4();
    const petMigration = migratePetCategory(true);
    saveState();
    initControls();
    render();
    const importParts = [`已导入 ${state.records.length} 条记录`];
